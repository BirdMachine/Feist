...
Push builds of `main` will not replace the release APK if any signing secret is missing. Such builds and pull-request builds without secrets use a disposable key for compilation checks only. After setup, dispatch the Android debug APK workflow on `main` to publish a stable-signed APK.
